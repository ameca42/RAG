...
Orchestrates the complete crawl pipeline: fetch stories -> content -> comments -> save.
"""
...
class HNCrawler:
    ...
    Fetches stories, content, comments and saves to storage.
    """

    def __init__(self, max_stories: int = None):
    ...
    async def crawl(self, skip_existing: bool = True) -> List[Dict[str, Any]]:
        ...
        app_logger.info("Step 1/4: Fetching story IDs from HN...")
        ...
        if skip_existing:
            ...
            app_logger.info(f"Step 2/4: Filtering - {len(new_ids)} new, {self.stats['skipped_existing']} existing")
            story_ids = new_ids
        else:
            app_logger.info("Step 2/4: Skip filtering disabled, processing all stories")

        # Step 3: Fetch story details
        app_logger.info(f"Step 3/4: Fetching {len(story_ids)} story details...")
        ...
        # Step 4: Fetch content and parse comments concurrently
        app_logger.info(f"Step 4/4: Fetching content and parsing comments for {len(stories)} stories...")
        ...
        # Prepare for storage
        prepared_articles = [prepare_article_for_storage(s) for s in stories_with_comments]
        ...
        failed_items = [
            ...
            for s in stories_with_comments
